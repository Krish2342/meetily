...
export function useSummaryGeneration({
  ...
  const processSummary = useCallback(async ({
    ...
    try {
      ...
      // Force small models to include the email by appending it to the custom prompt
      const emailDirective = "\n\nCRITICAL MANDATORY INSTRUCTION: You MUST output a section exactly titled '## Follow-Up Email Draft' at the very end of your response containing a professional follow-up email to the team.";
      const finalPrompt = customPrompt ? `${customPrompt}\n${emailDirective}` : emailDirective;
